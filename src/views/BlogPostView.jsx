...
import { blogPosts } from '../shared/data.js';

const BlogPostView = ({ navigate, postId }) => {
  ...
  if (!post) {
    return (
      <div className="w-full pt-32 px-6 md:px-16 lg:px-24 bg-[#101215] min-h-screen text-center">
        ...
        <Button onClick={() => navigate('blog')} variant="outline" className="mx-auto">Return to Blog</Button>
      </div>
    );
  }
  ...
  return (
    <div className="w-full pt-32 px-6 md:px-16 lg:px-24 bg-[#101215] min-h-screen pb-24">
      <div className="max-w-3xl mx-auto">
        ...
        {post.imageUrl && (
          <img src={post.imageUrl} alt={post.title} className="w-full h-64 md:h-96 object-cover rounded-lg mb-8 border border-[#2c303a]" />
        )}

...
const HomeView = ({ navigate }) => {
  ...
  return (
    <div className="w-full bg-[#101215]">
      ...
      <section className="min-h-[85vh] flex flex-col justify-center px-6 md:px-16 lg:px-24 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl fade-in-up">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <Button variant="outline" onClick={() => navigate('sponsors')}>Partner With Us</Button>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8">
          {[
            ...
            <div key={i} className="text-center fade-in-up" style={{ animationDelay: `${i * 0.1}s` }}>
              ...
              <div className="text-[#a9a9a9] text-sm uppercase tracking-wider font-mono">{stat.label}</div>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto text-center fade-in-up">
          ...
          {sponsorLogos.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8 opacity-60 grayscale hover:grayscale-0 transition-all duration-500">
               {/* Real logos mapped here when connected */}
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 opacity-50 max-w-5xl mx-auto">
              {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
                <div key={i} className="h-20 bg-[#1b1d23] border border-[#2c303a] rounded flex items-center justify-center text-[#d3d3d3] font-mono font-bold text-sm tracking-wider">
                  SPONSOR LOGO
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start max-w-7xl mx-auto">
          ...
          <div className="space-y-4">
            {flatAwards.map((award, i) => (
              <div key={i} className="flex items-start md:items-center gap-4 border-b border-[#2c303a] pb-4 text-left">
                <span className="text-[#5ddb27] font-mono text-xl md:text-2xl font-bold w-16 shrink-0">{award.year}</span>
                <span className="text-[#a9a9a9] font-mono text-sm uppercase tracking-wider w-32 shrink-0 hidden md:block">{award.event}</span>
                <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-3 text-white font-medium text-lg w-full">
                  <span className="text-[#a9a9a9] font-mono text-xs uppercase tracking-wider md:hidden">{award.event}</span>
          ...
          <div className="bg-[#101215] border border-[#2c303a] rounded-xl aspect-[4/3] lg:aspect-auto lg:h-full min-h-[400px] flex flex-col items-center justify-center relative overflow-hidden group">
            ...
            <ImageIcon size={64} className="text-[#2c303a] mb-4 z-20 group-hover:scale-110 transition-transform duration-500" />
      ...
      {/* 5. Current Projects Preview Section */}
      <section className="py-24 px-6 md:px-16 lg:px-24 bg-[#101215] border-t border-[#2c303a]">
        <div className="max-w-7xl mx-auto">
          <SectionHeader title="Ongoing Projects" subtitle="Making a direct impact in our community through continuous outreach and initiatives." />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
            <Card className="flex flex-col text-left !p-0 overflow-hidden !bg-[#1b1d23]">
              <div className="h-40 bg-[#2c303a] flex items-center justify-center border-b border-[#101215]">
                <span className="text-[#101215] font-mono font-bold tracking-widest text-xs">PROJECT IMAGE</span>
              </div>
      ...
      <section className="py-24 px-6 md:px-16 lg:px-24 bg-[#1b1d23] border-t border-[#2c303a]">
        <div className="flex justify-between items-end mb-12 fade-in-up max-w-7xl mx-auto">
          ...
          <button onClick={() => navigate('blog')} className="text-[#5ddb27] font-mono hover:underline mb-4 flex items-center gap-2 shrink-0">
          ...
          {blogPosts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {blogPosts.slice(0, 3).map((post) => (
                <Card key={post.id} onClick={() => navigate('blogPost', { postId: post.id })} className="flex flex-col h-full !p-0 overflow-hidden cursor-pointer !bg-[#101215]">
                  <div className="w-full h-48 bg-[#2c303a] relative flex items-center justify-center border-b border-[#1b1d23]">
                    {post.imageUrl ? (
                      <img src={post.imageUrl} alt={post.title} className="w-full h-full object-cover" />
                    ) : (
                      <span className="text-[#1b1d23] font-mono font-bold tracking-widest text-xs">BLOG IMAGE</span>
                    )}
                  </div>
                  <div className="p-6 flex flex-col flex-grow">

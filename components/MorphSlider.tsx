...
float softBox(vec2 p, vec2 center, vec2 halfSize, float feather) {
  vec2 d = abs(p - center) - halfSize;
  float distanceToBox = length(max(d, 0.0)) + min(max(d.x, d.y), 0.0);
  return 1.0 - smoothstep(0.0, feather, distanceToBox);
}
...
void main() {
  ...
  float ca = uReduce < 0.5 ? uAberration * env * 0.03 * transitionMask : 0.0;
...
class MorphEngine {
  ...
  private getProgress?: () => number | undefined;
  ...
  private raf = 0;
  private dirty = true;
  private lastControlledProgress = -1;
  ...
  private loop(t: number): void {
    const controlled = this.getProgress?.();
    if (controlled !== undefined && this.items.length > 1) {
      const nextProgress = Math.min(1, Math.max(0, controlled));
      if (this.dirty || Math.abs(nextProgress - this.lastControlledProgress) > 0.0001) {
        this.syncOptions();
        this.prepareNext(1);
        this.program.uniforms.uProgress.value = nextProgress;
        // A controlled morph should be deterministic: scroll position, not
        // wall-clock time, drives every shader input.
        this.program.uniforms.uTime.value = nextProgress * 2.0;
        this.renderer.render({ scene: this.mesh });
        this.lastControlledProgress = nextProgress;
        this.dirty = false;
      }
    } else {
...
export default function MorphSlider({
  ...
  useEffect(() => {
    ...
    const engine = new MorphEngine(containerRef.current, {
      ...
      startIndex,
      reducedMotion,
      dprCap: progressRef ? 1 : 2,
      getOptions: () => optsRef.current,
      onIndexChange: setIndex,
      getProgress: progressRef ? () => progressRef.current : undefined
    });

...
interface FluidGlassProps {
  ...
  textColor?: string;
  /** Render the bundled demo typography + image reel. Off = bare glass. */
  showcase?: boolean;
  /** Paint the opaque backdrop + full-viewport buffer plane. Off = see-through. */
  backdrop?: boolean;
  /** Image portaled into the lens's own scene so the glass has something to
      refract. Needed when showcase/backdrop are off, or the glass renders
      invisible (it can only bend what is inside its FBO scene). */
  sceneTexture?: string;
}
...
export default function FluidGlass({
  ...
  return (
    <Canvas
      ...
      style={{ backgroundColor: backdrop ? backgroundColor : 'transparent' }}
    >
      <ScrollControls damping={0.2} pages={3} distance={0.4}>
        {mode === 'bar' && <NavItems items={navItems as NavItem[]} textColor={textColor} />}
        <Wrapper modeProps={modeProps} backgroundColor={backgroundColor} backdrop={backdrop}>
          {showcase && (
            <>
              <Scroll>
          ...
          {sceneTexture && <SceneTexture url={sceneTexture} />}
          <Preload />
        </Wrapper>
      </ScrollControls>
    </Canvas>
...
function SceneTexture({ url }: { url: string }) {
  ...
  return (
    <Image
      url={url}
      scale={[viewport.width * 2, viewport.height * 2]}
      position={[0, 0, 8]}
      toneMapped={false}
    />
  );

...
// Ported from https://reactbits.dev/animations/ripple-distortion — adapted for our
// Next.js app (own component path, no other changes to the algorithm/shaders).
...
const QUALITY_DPR: Record<RippleQuality, number> = { low: 1, medium: 1.5, high: 2 };
...
const RippleDistortion = ({
  ...
  useEffect(() => {
    ...
    let inViewport = initialRect.bottom >= -100 && initialRect.top <= window.innerHeight + 100;
    ...
    const io = new IntersectionObserver(
      ([entry]) => {
        ...
        if (inViewport) needsRender = true;
      },
      { rootMargin: "100px" }
    );
    io.observe(mount);
    ...
    const onMove = (event: PointerEvent) => {
      const cfg = configRef.current;
      if (!inViewport || !cfg.enabled || reduceMotion || cfg.trigger === "click") return;
    ...
    const onDown = (event: PointerEvent) => {
      const cfg = configRef.current;
      if (!inViewport || !cfg.enabled || reduceMotion || cfg.trigger === "hover") return;
      const point = localPoint(event.clientX, event.clientY);
      if (!point) return;
    ...
    const loop = (now: number) => {
      ...
      if (!inViewport || document.hidden || !needsRender) {
        previousTime = now;
        return;
      }
